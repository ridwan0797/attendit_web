    ...
    <div class="q-pa-md shadow-md rounded-xl bg-white">
        ...
        <template v-slot:body-cell-approval="cell">
          <q-td class="text-center" v-bind="cell">
            <div v-if="cell.row.status === 'pending'" class="row justify-center">
              <q-btn round dense flat color="green" icon="done" @click="updateData(cell.row.id, 'approved')" />

              <q-btn round dense flat color="red" icon="close" @click="updateData(cell.row.id, 'rejected')" />
            </div>

            <div v-else-if="cell.row.status === 'approved'">
              <q-badge color="green">
                Approved
              </q-badge>
            </div>
      ...
      >
        <template v-slot:top-left>
          <div class="text-h6 text-weight-bold">Sick Data</div>
        </template>

        <template v-slot:item="props">
          <div class="bg-white w-2/5 p-3 mr-5 rounded-xl shadow-xl flex items-center justify-between mt-4">
  ...
  setup (props, vm) {
    ...
    const getDataId = (id, type) => {
      ...
      void api.get(urlUpdate)
        .then((response) => {
          ...
          console.log(record.value, type, 'type', 'getData')
        })
    }
    ...
    const getEmployee = (id) => {
      ...
      void api.get(urlBaru)
        .then((response) => {
          console.log(response, 'getData')
          nama.value = String(response.data.data.name)
          number.value = String(response.data.data.number)
        })
    }

    const params = new URLSearchParams()
    params.append('employee_id', record.value.employee_id)
    params.append('sick_dates', record.value.sick_date_dates)
    params.append('number_of_days', record.value.number_of_day)
    params.append('id', record.value.id)
    params.append('dates', '2022-09-09')
    ...
    const submitUpdate = (id) => {
      ...
      const urlBaru = 'http://localhost:3000/api/v1/sick-submission/' + id + '/approval'
      ...
      void api.put(urlBaru, stringify({
        ...
        .catch((error) => {
          console.log(error.response, 'error guys')
        })
    }
    ...
    const updateData = (id, type) => {
      console.log(id, type)
      getDataId(id, type)
      // record.value[0].permission_date_dates = Date.now()
      Dialog.create({
        title: 'Confirm',
    ...
    onMounted(() => {
      getEmployee(record.value.employee_id)
      getData()
    })
    return {
      onEmit,
